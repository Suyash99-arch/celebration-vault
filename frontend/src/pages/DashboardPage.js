...
<div class="dashboard-page page-enter">
    <section class="dashboard-hero" aria-labelledby="dashboard-title">
        <div class="hero-copy">
            <span class="eyebrow">YOUR CELEBRATION SPACE</span>
            <h1 id="dashboard-title">Make every milestone<br><em>feel remembered.</em></h1>
            <p>Keep the people you love close, with every birthday and anniversary beautifully in one place.</p>
            <div class="hero-actions">
                <button class="hero-btn" data-page="add">${icon("plus")}<span>Add birthday</span></button>
                <button class="hero-text-action" data-page="anniversary">Plan an anniversary ${icon("arrow")}</button>
            </div>
        </div>
        <div class="hero-orb hero-orb-one" aria-hidden="true"></div><div class="hero-orb hero-orb-two" aria-hidden="true"></div>
        <aside class="hero-highlight" aria-label="A thoughtful reminder">
            <span class="hero-highlight-icon">${icon("sparkle")}</span>
            <p>Small moments, beautifully remembered.</p>
            <span>Celebration Vault</span>
        </aside>
    </section>
    ...
    <section class="dashboard-panel upcoming-panel" aria-labelledby="upcoming-heading"><header class="section-heading"><div><span class="section-kicker upcoming-kicker">COMING UP</span><h2 id="upcoming-heading">Upcoming birthdays</h2></div><button data-page="members" class="section-link">Open directory ${icon("arrow")}</button></header><div id="recentMembers" class="member-grid" aria-live="polite"><div class="loading-card">Loading celebrations…</div></div></section>
</div>`, "dashboard");
}
